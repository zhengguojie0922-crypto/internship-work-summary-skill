...
Scale the deep-dive scope by route. For a named feature, fully expand that feature with approximately 15-20 evidence-supported core questions and 3-5 scenario questions. For Git discovery, fully expand the strongest two outputs by default using direct evidence, technical depth, and target-role relevance; summarize remaining verified outputs as resume-ready material and a concise appendix. When the user explicitly requests a comprehensive summary, expand additional outputs only while preserving complete answers. Evidence quality takes priority over breadth.

Before drafting career material, build an in-memory evidence matrix and shared feature model for each fully expanded output using the role analysis framework. Trace business scope, entry points, files and symbols, callers and callees, end-to-end code path, data and state, business rules, failures, recovery, configuration, tests, applicable Git evidence, design choices, alternatives, and evidence gaps. Every project-specific question must cite at least one concrete evidence anchor and explain the relationship among the evidence anchors. If candidate questions share substantially the same anchors, central conclusion, and answer path, merge duplicate questions and find a different evidence-backed angle.
...
2. `Feature Deep Dive`: explain `Business Function and User Value`, the original problem and failure sequence, architecture position, goals and non-goals, state/data/protocol model, happy and recovery paths, `Code Path`, error handling, alternatives, tests, and the code responsibility map. A path or line range alone is not analysis; connect symbols through control flow, data flow, state, or protocol behavior.
3. `Personal Work`: attributable actions, collaboration boundary, decisions, technical difficulties, validation, and explicit unknowns.
4. `Resume Wording`: three role-appropriate variants from [resume writing](references/resume-writing.md).
5. `Interview Introduction`: `30-second`, `1-minute`, and `3-minute` versions from [interview expansion](references/interview-expansion.md).
6. `Natural Interview Q&A`: use the interview reference's natural question/answer form. Start each main answer with a direct conclusion, then explain the system in connected paragraphs. Keep the former depth dimensions as an internal quality rubric rather than visible fields. Each follow-up must be shown and answered separately with a complete standalone answer.
7. `Scenario Questions`: add 3-5 scenario questions for every fully expanded output, each with a complete natural answer and separately answered follow-ups.
...
Use the resume and interview references to expand these sections. Keep every factual claim evidence-backed and every stronger unsupported claim in the explicit unknowns or metrics section. When evidence is insufficient, reduce breadth; never pad or fabricate to satisfy an approximate question count. Before the single final write, run a final quality audit in memory: confirm that the feature model explains the system, code relationships are proved rather than listed, main answers are coherent and direct, follow-ups are independently answered, questions cover distinct technical dimensions, attribution is consistent across resume and interview material, and scenarios distinguish current implementation from proposed action. Return to the evidence matrix or feature model and repair any failed item before writing the document.
